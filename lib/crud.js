...
module.exports = class CRUD {
  ...
  authenticate(request, res) {
    ...
    let hashFields = process.env.APP_AUTH_FIELDS_HASH.split(',');
    ...
    for (var j = hashFields.length - 1; j >= 0; j--) {
      query += ' AND `' + hashFields[j] + '` = MD5(' + db.escape(request.values[fields.length + j]) + ')';
    }
    ...
    try {
      db.query(query, (error, result) => {
        if (error) {
          ...
          if (request.hiddenFields !== undefined && request.hiddenFields.length > 0) {
            for (var object in result) {
              for (var field in result[object]) {
                if (request.hiddenFields.includes(field)) {
                  result[object][field] = '[HIDDEN]';
                }
              }
            }
          }
